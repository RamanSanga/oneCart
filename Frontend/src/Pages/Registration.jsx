...
export default function SignupPage() {
  ...
  const handleSignup = async e => {
    ...
    setError("");
    try {
      await axios.post(`${serverUrl}/api/auth/registration`, { name: form.name, email: form.email, password: form.password }, { withCredentials: true });
    ...
    <div className="min-h-screen flex bg-[var(--cream)] text-[var(--ink)]">
      ...
      <div className="hidden lg:block lg:w-1/2 relative overflow-hidden">
        ...
        <div className="absolute bottom-12 left-10">
          ...
          <p className="text-white font-display font-light italic text-2xl leading-snug">
            Join the edit.<br />Your style, curated.
          </p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 md:p-16">
        <div className="w-full max-w-sm">
          ...
          <p className="text-[9px] font-semibold uppercase tracking-[0.3em] text-[var(--ink-40)] mb-4">Create account</p>
          ...
          <form onSubmit={handleSignup} className="space-y-8">
            <div>
              <label className="block text-[9px] font-semibold uppercase tracking-[0.25em] text-[var(--ink-40)] mb-2">Full Name</label>
              <input
          ...
          {/* Google */}
          <button
            type="button" onClick={googleSignUp} disabled={googleLoading}
          ...
          <p className="mt-10 text-[12px] font-light text-[var(--ink-60)]">
            ...
            <Link to="/login" className="font-medium text-[var(--ink)] border-b border-[var(--ink)] pb-0.5 hover:text-[var(--ink-60)] transition-colors">
              Sign in →
            </Link>
          </p>
        </div>
